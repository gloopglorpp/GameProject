...
## 2026-06-10 — Adding Curious Fireflies Around the Player

The scene already had a strong sunset background, but it needed a small moving detail close to the character. Instead of putting fireflies everywhere in the world, I added a small group that stays near the player. This makes them feel more like little creatures reacting to the character than generic background particles.

Each firefly is its own small state object. It stores its position, orbit angle, orbit radius, orbit speed, jitter timing, pulse timing, follow strength, and dart behavior. This means the fireflies do not move in a perfect circle together. They hover at different distances and speeds, which makes the group feel less symmetrical and more alive.

The player rectangle is used to calculate a target near the head. The fireflies orbit around that target, then ease toward their desired position instead of snapping to it. This loose following behavior makes them feel curious and slightly annoying, like they are buzzing around the character rather than being attached to him.

I also added short dart movements. Sometimes a firefly temporarily shrinks its orbit radius so it zips toward the character's head. Other times it expands the radius and backs away. Because these moments happen at different times for each firefly, the group feels playful instead of mechanical.

Visually, each firefly has a soft amber glow and a brighter core. The glow pulses over time using each firefly's own pulse speed and phase. This keeps the effect small, warm, and readable against the dark sunset scene.

### Lessons Learned

- Small creatures can be represented with simple state dictionaries.
- A target point based on the player rectangle keeps effects attached to gameplay objects.
- Loose following feels more natural than instantly snapping to a target.
- Variation in speed, radius, pulse, and jitter prevents symmetrical movement.
- Occasional special behavior, such as darting, can make a simple ambient effect feel intentional.
